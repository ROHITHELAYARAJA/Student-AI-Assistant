...
test('transient failure retries same model once then uses fallback',async()=>{const ids=[];const r=await routeStructured(base,deps(async(_s,_m,id)=>{ids.push(id);if(ids.length<=2)throw Object.assign(Error('private provider message'),{name:'ServiceUnavailableException'});return {text:'{"answer":"valid"}',modelId:id};}));assert.equal(r.routing.attempts,3);assert.deepEqual(ids,['xai.grok-4.6','xai.grok-4.6','zai.glm-5']);});
...
test('vision failures never fall through to text-only models',async()=>{const seen=[];await assert.rejects(routeStructured({...base,context:{prompt:'Read figure',hasImages:true},messages:[{role:'user',text:'Read figure',images:[{format:'png',bytes:Buffer.from([1]),sourceId:'p1'}]}]},deps(async(_s,m,id)=>{seen.push(id);assert.equal(m[0].images.length,1);throw new ModelFailure('busy',true);},models.filter(m=>m.id==='moonshotai.kimi-k2.5'||!m.vision))));assert.deepEqual(seen,['moonshotai.kimi-k2.5','moonshotai.kimi-k2.5']);});
