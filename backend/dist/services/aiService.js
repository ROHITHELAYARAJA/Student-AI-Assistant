...
exports.processStudyRequest = processStudyRequest;
const operationsList_js_1 = require("./operationsList.js");
const structuredParser_js_1 = require("./structuredParser.js");
async function processStudyRequest(request) {
    const startTime = Date.now();
    const opMeta = operationsList_js_1.ALL_OPERATIONS.find((op) => op.id === request.operation) || {
            ...
            const res = await fetch(groqUrl, {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        {
                            role: 'system',
                            content: 'You are an elite, highly articulate academic and coding tutor. Provide structured, accurate, beginner-accessible yet comprehensive responses. Do NOT add unnecessary conversational fluff.'
                        },
    ...
    return (0, structuredParser_js_1.parseMarkdownToStructured)(opMeta.id, opMeta.outputComponent, request.studyTopic || request.researchGoal || opMeta.name, rawMarkdown, modelUsed, startTime);
}
function buildSystemAndUserPrompt(req, opName) {
...
function generateFallbackContent(req, opId, opName) {
    ...
    if (opId === 'quiz' || opId === 'check_understanding' || opId === 'exam_questions' || opId === 'mock_interview') {
        return (`# ${topic} Mastery Assessment\n\n` +
    ...
    if (opId === 'code' ||
        ...
        opId === 'datastructure' ||
        opId === 'pattern') {
        return (`# Production Solution: ${topic}\n\n` +
    ...
    if (opId === 'compare' || opId === 'pros_cons' || opId === 'code_compare' || opId === 'debate' || opId === 'resume_tips') {
        return (`# Comprehensive Comparison: ${topic} Frameworks\n\n` +
    ...
    if (opId === 'study_plan' || opId === 'timeline' || opId === 'revision' || opId === 'roadmap' || opId === 'career_path') {
        return (`# 7-Day Mastery Plan: ${topic}\n\n` +
    ...
    if (opId === 'formulas' ||
        ...
        opId === 'chemistry' ||
        opId === 'stats') {
        return (`# Mathematical Formulation: ${topic}\n\n` +
