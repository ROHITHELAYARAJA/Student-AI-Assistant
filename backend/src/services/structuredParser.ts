...
function extractFlashcards(lines: string[], defaultTitle: string): FlashcardItem[] {
...
function extractQuiz(raw: string, defaultTitle: string): QuizQuestion[] {
...
function extractCode(raw: string): CodeBlockData {
...
function extractComparison(raw: string, defaultTitle: string): {
  entityA: string;
  entityB: string;
  rows: ComparisonRow[];
  verdict: string;
  ...
  if (rows.length === 0) {
    rows.push(
      { aspect: 'Performance & Speed', itemA: 'High throughput, zero overhead', itemB: 'Moderate latency, higher abstractions', verdict: `${entityA} leads in raw compute` },
      { aspect: 'Memory Utilization', itemA: 'Strict, optimized footprint', itemB: 'Dynamic garbage-collected buffers', verdict: `${entityA} is more predictable` },
      { aspect: 'Developer Productivity', itemA: 'Steeper learning curve', itemB: 'Rapid prototyping and ecosystem', verdict: `${entityB} allows faster iteration` },
      { aspect: 'Scalability & Maintenance', itemA: 'Excellent concurrency support', itemB: 'Requires distributed coordination', verdict: 'Balanced trade-off' },
      { aspect: 'Best Suited For', itemA: 'High-frequency systems and exams', itemB: 'Full-stack enterprise applications', verdict: 'Context dependent' }
    );
  }
...
function extractTimeline(lines: string[], defaultTitle: string): TimelineDay[] {
...
function extractFormulas(raw: string, defaultTitle: string): FormulaItem[] {
  const formulas: FormulaItem[] = [];
  const lines = raw.split('\n').map((l) => l.trim()).filter(Boolean);

  let currentFormula: FormulaItem | null = null;
...
function extractMindmap(lines: string[], defaultTitle: string): MindmapNode {
...
function extractKeypoints(lines: string[]): KeypointItem[] {
...
function extractArticle(raw: string): {
  sections: Array<{ heading: string; body: string; highlights?: string[] }>;
} {
  const rawSections = raw.split(/(?=\n#{1,3}\s+)/);
  const sections: Array<{ heading: string; body: string; highlights?: string[] }> = [];
